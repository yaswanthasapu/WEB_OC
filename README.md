...
When `reports.playbackNetworkDiagnostics` is `true`, every parent/direct-event Play action captures sanitized network diagnostics. After Live View identifies the Unit ID, the JSON is attached beside `Unit ID <id> - Site Info Screenshot` and `Unit ID <id> - Live View Screenshot` as `Unit ID <id> - Playback Network Diagnostics`. Child events do not create playback-network attachments. The JSON includes Event Type/Tag, camera identity, detected video transport, media response status and content type, exposed server headers, failed media requests, WebSocket activity, and `<video>` state. URL credentials, query strings, and fragments are removed. Set `reports.junitStdout` to `false` to remove `<system-out>` from JUnit XML after the run; HTML attachments remain available.
